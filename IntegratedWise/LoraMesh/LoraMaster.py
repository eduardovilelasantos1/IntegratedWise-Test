...
# ================================================================
#  IMPORTAÇÃO DO GERENCIADOR DE ALARMES  (ÚNICA ADIÇÃO REAL)
# ================================================================
...
# =====================================================
#  ⭐ NOVO: ARQUIVO communication_time.json
# =====================================================
COMM_TIME_FILE = os.path.join(PROJECT_ROOT, "LoraMesh", "communication_time.json")

def save_comm_time():
    """Salva o tempo sem comunicação no communication_time.json"""
    try:
...
def solicitar_rssi(ser, target_id=SLAVE_ID, timeout=0.35):
    try:
        ...
        frame = bytearray([target_id & 0xFF, (target_id >> 8) & 0xFF, CMD_RSSI, 0x00])
        ...
        header = bytes([target_id & 0xFF, (target_id >> 8) & 0xFF, CMD_RSSI])
...
def main():
    ...
    while True:
        ...
        # Atualiza tempo sem comunicação
        save_comm_time()

        try:
        
            # FLAG de reconfiguração
            ...
            # ============================================================
            # ⭐ ADIÇÃO MINIMA — AVALIA ALARMES + SALVA communication_time.json
            # ============================================================
                ...
                if ser.in_waiting > 0:
                    ...
                    if start_idx != -1 and (len(serial_buffer) - start_idx) >= FRAME_FULL_SIZE:
                        ...
                        try:
                            ...
                            # ⭐ Atualiza tempo sem comunicação (reset)
