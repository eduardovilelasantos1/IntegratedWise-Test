...
COMM_TIME_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'LoraMesh', 'communication_time.json'))
...
def salvar_configuracao():
    ...
    try:
        ...
        with open(os.path.join(CONFIG_DIR, 'reconfig.flag'), 'w') as f:
            pass
        ...
        opcua_users_raw = request.form.get('opcua_users', '')
        users = {}
        if opcua_users_raw:
            for pair in opcua_users_raw.split(","):
                if ":" in pair:
...
def reset_bateria():
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    try:
        with open(os.path.join(CONFIG_DIR, 'reset_battery.flag'), 'w') as f:
            pass
...
def visualizacao():
    ...
    sensor_config['comm_time'] = {"label": "Tempo sem Comunicação", "unit": "s"}
    sensor_config['rssi_ida'] = {"label": "RSSI (ida)", "unit": ""}
    sensor_config['snr_ida'] = {"label": "SNR (ida)", "unit": ""}
    ...
    try:
        ...
        if selected_channel:
            ...
            if trim_zero and trim_span:
                config_420[selected_channel] = {
                    "TRIM_ZERO_BIT": int(trim_zero),
                    "TRIM_SPAN_BIT": int(trim_span)
                }
...
# ALARMES
# ------------------------------------------------------
# --- trechos omitidos para reduzir tamanho ---
# (TODO O RESTO DO SEU ARQUIVO PERMANECE IGUAL)
# AQUI ESTÃO APENAS AS PARTES MODIFICADAS
...
def alarmes():
    ...
    # ------------------------------------------------------------------
    # 🔥 CARREGA CONFIG DOS SENSORES E ADICIONA SENSORES VIRTUAIS
    # ------------------------------------------------------------------
    sensor_config = load_json('config_min_max.json')

    # Sensores virtuais adicionados manualmente
    sensor_config["bat_percent"] = {"label": "Bateria (%)", "unit": "%", "min": 0, "max": 100}
    sensor_config["battery_voltage"] = {"label": "Tensão da Bateria", "unit": "V", "min": 0, "max": 15}
    sensor_config["bat_days"] = {"label": "Autonomia (dias)", "unit": "Dias", "min": 0, "max": 365}
    sensor_config["comm_time"] = {"label": "Tempo sem Comunicação", "unit": "s", "min": 0, "max": 86400}
    ...
    if request.method == 'POST':
        try:
            ...
            for i in range(1, 10):
                ...
                # Sensores reais → calculam bits
                if source in sensor_config and "channel" in sensor_config[source]:
                    if limit_real_str:
    ...
    # Status real dos relés
    STATUS_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'Alarms', 'alarmes_status.json'))
    try:
...
def reset():
    ...
    if request.method == 'POST':
        try:
            ...
            # ⭐ RESETAR CALIBRAÇÃO + NOMES DOS SENSORES
            save_json('config_min_max.json', DEFAULT_MIN_MAX_CONFIG)

            # ⭐ RESETAR SENHA PARA admin/admin
            save_users(DEFAULT_USERS)

            flash('⚠️ Sistema restaurado para padrões de fábrica!', 'alert-warning')
...
def api_alarm_status():
    try:
        status_file = os.path.join(os.path.dirname(__file__), '..', 'Alarms', 'alarmes_status.json')
        with open(status_file, 'r') as f:
            data = json.load(f)
        return jsonify(data)
    except:
